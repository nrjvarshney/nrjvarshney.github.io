...
class NLIDataset(Dataset):
    def __init__(self, tokenizer, df_file_name, input_max_len=512, max_samples=-1, label2id={}, id2label={}):
        ...
        all_labels = list(self.df["writer_id"].unique())
        all_labels.sort()
        self.label2id = label2id
        self.id2label = id2label

        self.df["writer_id"].replace(self.label2id, inplace=True)

    def __getitem__(self, index):
        sentence1 = str(self.df.iloc[index]["text"])

        sentence2 = None

        label = self.df.iloc[index]["writer_id"]
...
def compute_label_mappings(df_file_name):
    df = pd.read_csv(df_file_name)
    all_labels = list(df["writer_id"].unique())
    all_labels.sort()
    label2id = {}
    id2label = {}
...
def evaluate(model, other_arguments, label2id, id2label):
    ...
    with torch.no_grad():
        ...
        softmax_logits_df = pd.DataFrame(all_softmax_logits)
        softmax_logits_df = softmax_logits_df.rename(columns=id2label)
        print("--------------------")
        print("Validation avg_loss: ", avg_loss)
        print("Validation avg_acc: ", avg_acc)
        ...
        result_df["label"].replace(id2label, inplace=True)
        result_df["prediction"].replace(id2label, inplace=True)
        result_df = pd.concat([result_df, softmax_logits_df], axis=1)

        if (other_arguments.write_dev_predictions):
            output_path = other_arguments.output_dir + "epoch_" + other_arguments.predictions_file
...
if __name__ == "__main__":
    ...
    label2id, id2label = compute_label_mappings(other_arguments.TRAIN_FILE)
    model = ClassificationModel.load_from_checkpoint(checkpoint_path=other_arguments.checkpoint_path,
                                                     other_arguments=other_arguments)

    evaluate(model, other_arguments, label2id=label2id, id2label=id2label)
